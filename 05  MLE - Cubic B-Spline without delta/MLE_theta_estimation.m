...
function [theta_hat, log_lik, M_vec] = MLE_theta_estimation( ...
    Smooth_AllR, Smooth_AllR_RND, Realized_Return, Risk_Free_Rate, b)

    % Settings
    rng(0);
    theta0 = randn(1, b+1) * 0.1;
    LB = [];
    UB = [];
    ...
    [~, M_vec] = log_likelihood_spline(theta_hat, R_vec, Rf_vec, ...
    Smooth_AllR, Smooth_AllR_RND, dates, b);
...
function [LL, M_vec] = log_likelihood_spline(theta, R_vec, Rf_vec, ...
    Smooth_AllR, Smooth_AllR_RND, dates, b)
    ...
    for t = 1:T
        ...
        M_grid  = exp(g_vec);                                              % (1*30000) Pricing kernel M(R_{t+1}; θ)
        M_vec(t, :) = M_grid;
        ...
        if R_realized_t < min(R_axis) || R_realized_t > max(R_axis)
            f_physical_at_realized = 1e-12;                                % Penalize out-of-bound realized returns
        else
            f_physical_at_realized = interp1(R_axis, f_curve, R_realized_t, 'linear', 'extrap');
