...
%% Estimation: MLE over (alpha, beta, b) - Quinic B-Spline

clc

% Add paths
Path_Code_14 = fullfile(Path_MainFolder, 'Code', ...
    '14  MLE - B-Spline with Fixed Initial Wealth');
addpath(Path_Code_14);
...
parfor i = 1:Total_Tasks
    ...
    outname = sprintf('W0_%d_b_%d_alpha_%.2f_beta_%.2f.mat', initial_wealth, b_val, alpha, beta);
    OutputFile = fullfile(Path_Output, outname);    
    fprintf('Task Running: W0=%d, b=%d, alpha=%.2f, beta=%.2f\n', initial_wealth, b_val, alpha, beta);
    ...
    [theta_hat, log_lik, BIC] = MLE_BSpline_estimation( ...
        ...
        initial_wealth, b_val, use_delta, alpha, beta, D.Global_Min_R, D.Global_Max_R);
    ...
    Result.Global_Min_R   = D.Global_Min_R;
    Result.Global_Max_R   = D.Global_Max_R;
    Result.min_knot       = D.Global_Min_R * initial_wealth;
    Result.max_knot       = D.Global_Max_R * initial_wealth;
