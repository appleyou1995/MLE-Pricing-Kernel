...
Path_Output      = fullfile(Path_MainFolder, 'Code', '14  Output');
Path_Output_Plot = fullfile(Path_MainFolder, 'Code', '14  Output - Plot');
...
k_order = n_degree + 1;
tol = 1e-9;
files = dir(fullfile(Path_Output, sprintf('W0_%d_b_*.mat', initial_wealth)));
...
for g = 1:numel(plot_groups)
    ...
    for plot_mode = 1:2
        ...
        for k = 1:numel(group.indices)
            ...
            for f = 1:numel(files)
                try
                    S = load(fullfile(Path_Output, files(f).name), ...
                        'b_val','alpha','beta','theta_hat', ...
                        'min_knot', 'max_knot', 'initial_wealth');
                catch, continue; end
            ...
            current_w0 = S.initial_wealth;
            
            % --- B-spline 設定 ---  
            k_order_current = n_degree + 1;
            num_basis_function = p.b + 1;
            num_breaks = num_basis_function - k_order_current + 2;
            breaks = linspace(S.min_knot, S.max_knot, num_breaks);
            knots  = augknt(breaks, k_order_current);
            
            sp = spmak(knots, theta_hat');
            Spline_Val = fnval(sp, R_axis * S.initial_wealth);
            ...
            if plot_mode == 1
                ...
                x_data = R_axis * current_w0;
            end

            plot(x_data, M_curve, 'LineWidth', 2, ...
                'Color', colors(k,:), ... 
        ...
        if plot_mode == 1
            ...
            ylabel('$M_t(R_{t+1})$');
            xlim([0.8 1.2]);
            out_name = sprintf('plot_M_curve_R_%s.png', group.suffix);
        else
            xlabel('$w_0 R_{t+1}$');
            ylabel('$M_t(w_0 R_{t+1})$');
            xlim([0.8 * current_w0, 1.2 * current_w0]);
            out_name = sprintf('plot_M_curve_W_%s.png', group.suffix);
        end
...
for g = 1:numel(plot_groups)
    ...
    for k = 1:numel(group.indices)
        ...
        for t = 1:T_len
            ...
            Basis_Precomputed{t} = spcol(knots, k_order_current, R_axis_t(:) * S.initial_wealth);
        end
    ...
    out_name = sprintf('plot_delta_%s.png', group.suffix);
    exportgraphics(fig, fullfile(Path_Output_Plot, out_name), 'Resolution', 300);
    hold off;
end
...
for idx = 1:length(param_list)
    ...
    W_axis = R_axis * S.initial_wealth;
    
    % 計算導數
    S_0_raw = fnval(sp, W_axis);             
    S_1_raw = fnval(fnder(sp, 1), W_axis);
    S_2_raw = fnval(fnder(sp, 2), W_axis);
    S_3_raw = fnval(fnder(sp, 3), W_axis);
    ...
    if use_w0_axis
        ...
        target_R_points = [0.8, 1.0, 1.2] * S.initial_wealth;
        csv_bounds      = [1.09, 1.11] * S.initial_wealth;
        range_bounds    = [0.8, 1.2] * S.initial_wealth;
        file_suffix     = '_W0';
    else
        X_data = R_axis;
        col_name_X = 'R';
        chain_w = S.initial_wealth;
        
        target_R_points = [0.8, 1.0, 1.2];
        csv_bounds      = [1.09, 1.11];
    ...
    S_0 = S_0_raw;
    S_1 = S_1_raw * chain_w;
    S_2 = S_2_raw * (chain_w^2);
    S_3 = S_3_raw * (chain_w^3);
    ...
    risk_pref = struct();
    risk_pref.ARA = -S_1; 
    risk_pref.RRA = X_data .* risk_pref.ARA;
    risk_pref.AP  = -M2 ./ M1;
    risk_pref.RP  = X_data .* risk_pref.AP;
    risk_pref.AT  = -M3 ./ M2;
    risk_pref.RT  = X_data .* risk_pref.AT;
    
    risk_results{idx} = risk_pref;
    
    % 輸出 Table (CSV)
    T_out = table(X_data, M_val, M1, M2, M3, ...
                   risk_pref.ARA, risk_pref.RRA, ...
                   risk_pref.AP, risk_pref.RP, ...
                   risk_pref.AT, risk_pref.RT, ...
                   'VariableNames', {'R', 'M', 'M1', 'M2', 'M3', ...
                                     'ARA', 'RRA', 'AP', 'RP', 'AT', 'RT'});
    mask_csv = (X_data >= csv_bounds(1)) & (X_data <= csv_bounds(2));
    ...
    csv_name = sprintf('RiskTable_alpha_%.2f_beta_%.2f%s.csv', p.alpha, p.beta, file_suffix);
    ...
    % 1. 建立遮罩，用來抓取區間內所有數據點
    range_mask = (X_data >= range_bounds(1)) & (X_data <= range_bounds(2));
    
    % 2. 計算特定點的值
    for k = 1:length(target_R_points)
        val_target = target_R_points(k);
        col_name = sprintf('Val_at_%s', strrep(num2str(val_target), '.', '_'));
        
        vals = zeros(length(measures), 1);
        for m = 1:length(measures)
            key = measures{m};
            y_data = risk_pref.(key);
            vals(m) = interp1(X_data, y_data, val_target, 'pchip');
        end
        trend_data.(col_name) = vals;
    end
    
    % 3. 新增：找出區間內真正的最大與最小值 (解決微笑曲線問題)
    ...
    trend_data.Min_Range = min_vals;
    trend_data.Max_Range = max_vals;
    ...
    % 儲存檔案
    trend_csv_name = sprintf('TrendCheck_b_%d_alpha_%.2f_beta_%.2f%s.csv', ...
        p.b, p.alpha, p.beta, file_suffix);
...
if use_w0_axis
    X_plot_data = R_axis * initial_wealth;
    x_label_str = '$w_0 R_{t+1}$';
    x_lim_vals  = [0.8 * initial_wealth, 1.2 * initial_wealth];
    file_suffix = '_W0';
else
    X_plot_data = R_axis;
    x_label_str = '$R_{t+1}$';
...
for m = 1:length(measures)
    ...
    for g = 1:numel(plot_groups)
        ...
        for k = 1:numel(group.indices)
            ...
            legend_str = sprintf('$\\alpha=%.2f, \\,\\beta=%.2f$', p.alpha, p.beta);
            plot(X_plot_data, Y_plot, 'LineWidth', 2, 'Color', colors(k,:), 'DisplayName', legend_str);
        end
        ...
        out_name = sprintf('plot_Risk_%s_%s%s.png', measure_key, group.suffix, file_suffix);
        exportgraphics(fig, fullfile(Path_Output_Plot, out_name), 'Resolution', 300);
        close(fig);
    end
end
...
for idx = 1:length(param_list)
    ...
    for f = 1:numel(files)
        try
            S = load(fullfile(Path_Output, files(f).name), 'b_val','alpha','beta');
        catch, continue; end
        
        if S.b_val == b_target && ...
           abs(S.alpha - alpha_target) < tol && ...
           abs(S.beta - beta_target) < tol
...
for i = 1:length(param_list)
    ...
    for f = 1:numel(files)
        ...
        try
            S = load(Sfile, 'b_val', 'alpha', 'beta'); 
        catch
            continue;
        end
        
        if S.b_val == b_tar && ...
           abs(S.alpha - alpha_tar) < tol && ...
           abs(S.beta - beta_tar) < tol
